...
class TMVIPController: TMBaseViewController {
    
    private var vipItems = [ComicListModel]()
    ...
    func loadData() {
        ApiLoadingProvider.request(TMAPI.vipList, model: VipListModel.self) { (returnData) in
            self.collectionView.mj_header?.endRefreshing()
            self.vipItems = returnData?.newVipList ?? []
...
extension TMVIPController: TMCollectionViewSectionBackgroundLayoutDelegate, UICollectionViewDataSource {
    
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return vipItems.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            let comicList = vipItems[indexPath.section]
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        let comicList = vipItems[section]
        return comicList.itemTitle?.count ?? 0 > 0 ? CGSize(width: kScreenWidth, height: 44) : CGSize.zero
    }
    
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForFooterInSection section: Int) -> CGSize {
        return vipItems.count - 1 != section ? CGSize(width: kScreenWidth, height: 10) : CGSize.zero
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let comic = vipItems[indexPath.section]
