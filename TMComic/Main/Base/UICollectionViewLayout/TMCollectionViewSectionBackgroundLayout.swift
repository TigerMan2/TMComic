//
//  TMCollectionViewSectionBackgroundLayout.swift
//  TMComic
//
//  Created by Luther on 2020/5/15.
...
protocol TMCollectionViewSectionBackgroundLayoutDelegate: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView,
    layout collectionViewLayout: UICollectionViewLayout,
    backgroundColorForSectionAt section: Int) -> UIColor
}

extension TMCollectionViewSectionBackgroundLayoutDelegate {
    func collectionView(_ collectionView: UICollectionView,
                        layout collectionViewLayout: UICollectionViewLayout,
                        backgroundColorForSectionAt section: Int) -> UIColor {
...
private class TMCollectionViewLayoutAttributes: UICollectionViewLayoutAttributes {
    var backgroundColor = UIColor.white
    
    override func copy(with zone: NSZone? = nil) -> Any {
        let copy = super.copy(with: zone) as! TMCollectionViewLayoutAttributes
        copy.backgroundColor = self.backgroundColor
        return copy
    }
    
    override func isEqual(_ object: Any?) -> Bool {
        guard let rhs = object as? TMCollectionViewLayoutAttributes else {
            return false
        }
...
private class TMCollectionReusableView: UICollectionReusableView {
    
    override func apply(_ layoutAttributes: UICollectionViewLayoutAttributes) {
        super.apply(layoutAttributes)
        
        guard let attr = layoutAttributes as? TMCollectionViewLayoutAttributes else {
            return
        }
...
class TMCollectionViewSectionBackgroundLayout: UICollectionViewFlowLayout {
    ...
    func setup() {
        self.register(TMCollectionReusableView.classForCoder(), forDecorationViewOfKind: SectionBackground)
    }
    ...
    override func prepare() {
        ...
        guard let numberOfSections = self.collectionView?.numberOfSections,
            let delegate = self.collectionView?.delegate as? TMCollectionViewSectionBackgroundLayoutDelegate
            else {
                return
        }
        ...
        for section in 0..<numberOfSections {
            ...
            let attr = TMCollectionViewLayoutAttributes(forDecorationViewOfKind: SectionBackground, with: IndexPath(item: 0, section: section))
