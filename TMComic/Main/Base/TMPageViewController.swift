...
class TMPageViewController: TMBaseViewController {
    
    var pageStyle: TMPageStyle!
    
    lazy var segment: HMSegmentedControl = {
        return HMSegmentedControl().then { (make) in
            make.addTarget(self, action: #selector(changeIndex(segment:)), for: .valueChanged)
        }
    }()
    ...
    convenience init(titles: [String] = [], vcs: [UIViewController] = [], pageStyle: TMPageStyle = .none) {
        self.init()
        self.titles = titles
        self.vcs = vcs
    ...
    override func setupSubviews() {
        ...
        case .navgationBarSegment?:
            ...
            segment.frame = CGRect(x: 0, y: 0, width: kScreenWidth - 120, height: 40)
            pageVC.view.snp.makeConstraints{ $0.edges.equalToSuperview() }
        case .topTabBar?:
...
extension TMPageViewController: UIPageViewControllerDelegate,UIPageViewControllerDataSource {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        guard titles != nil && pageStyle == TMPageStyle.none else {
            return
        }
